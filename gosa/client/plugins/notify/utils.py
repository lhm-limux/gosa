...
class Notify(Plugin):
    ...
    def notify(self, title, message, user):
        ...
        return gosa_dbus.notify(title, message, user, dbus_interface="com.gonicus.gosa")

    @Command()
    def notify_user(self, title, message, user):
        """ Sent a notification to a given user """
        bus = dbus.SystemBus()
        gosa_dbus = bus.get_object('com.gonicus.gosa',
                                   '/com/gonicus/gosa/notify')
    ...
    @Command()
    def notify_all(self, title, message, user):
        """ Sent a notification to a given user """
        bus = dbus.SystemBus()
        gosa_dbus = bus.get_object('com.gonicus.gosa',
                                   '/com/gonicus/gosa/notify')
